...
/*   Updated: 2022/04/03 02:36:19 by iyamada          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	update_last_mealtime(t_philo *philo)
{
	...
	if (mutex_lock(&philo->time_mtx, &philo->mtxes[ERR], philo->err) == FAIL)
		return (FAIL);
	philo->last_meal_time = get_timestamp();
	return (mutex_unlock(&philo->time_mtx, &philo->mtxes[ERR], philo->err));
}
